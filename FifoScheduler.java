...
import java.util.Comparator; // comparator added
...
public class FifoScheduler implements ResourceScheduler {
  ...
  private static final int MINIMUM_PROCESSES = 10;
  ...
  private Integer no_of_process ;
  private Resource minimumAllocation;
  private Resource maximumAllocation;
 //private Comparator nodeAvailableResourceComparator =  new NodeAvailableResourceComparator(); // Node available resource comparator

   //private Comparator FitnessComparator =  new FitnessComparator(); // job fitness comparator
  ...
  //----------------------------------hash map o(1) search and insert---------------------------------------------------- 
  private Map<ApplicationAttemptId,SchedulerApp> applications = new ConcurrentHashMap<ApplicationAttemptId,SchedulerApp>();
  ...
  throws IOException 
  {
    if (!this.initialized) {
      this.conf = conf;
      this.no_of_process = 10 ;//conf.getInt(NO_OF_PROCESSES,MINIMUM_PROCESSES);
...
/*
    -------------------------------------------TO DO---------------------------------------
      applications ko treemap se hash map mei karna hai   done !
    this.no_of_process = conf.getInt(NO_OF_PROCESSES,MINIMUM_PROCESSES) =>  add no_of_processes to conf file  
     get system arguments for MPI initalization and pass it to the HPC_MPJ obj done !
---------------------------------------------------------------------------------------------  
  */
  ...
  applications : list of all applications Map.Entry<ApplicationAttemptId, SchedulerApp> => ConcurrentHashMap<ApplicationAttemptId,SchedulerApp>
  
headroom = application ki resource limit se kitna door hai current resources using => app.getHeadroom() : Resource 
reservated resources by an app =>  app.getCurrentReservation()  : Resource type
final Map<Priority, Map<String, ResourceRequest>> requests =new HashMap<Priority, Map<String, ResourceRequest>>();
  ...
  //-------------------Functions-----------------------------------
    // /*
      private List<SchedulerApp> get_fitness_sorted_Applications(SchedulerNode node)
    {
      // Collection<SchedulerApp> apps_collection = applications.values();
      // List<SchedulerApp> app_list = new ArrayList<SchedulerApp>(apps_collection);
      // HashMap<SchedulerApp,Integer> fitness_map = new HashMap<SchedulerApp,Integer>();

      // for (SchedulerApp app : app_list)
      // {
      //   long fitness = getFitness(app);
      //   fitness_map.add()
      // }
      int indx=0;
        ChainHash fitness_applist = new ChainHash();
        List<SchedulerApp> result = new ArrayList<SchedulerApp>();
        float fitness_arr[] = new float[applications.size()];
       // Map<SchedulerApp,Float> fitness_map = new ConcurrentHashMap<SchedulerApp,Float>();
      for (Map.Entry<ApplicationAttemptId, SchedulerApp> e : applications.entrySet()) 
      {
        SchedulerApp application = e.getValue();
        float fitness = getFitness(application,node);
        //fitness_map.put(application,fitness);
        fitness_applist.add(fitness,application);
       // fitness_map_inv.put(fitness,application);
        fitness_arr[indx++] = fitness;
      }
     // float fitness_arr[] = fitness_map.values().toArray(new float[fitness_map.size()]);
      HPC_MPJ mpjobj = new HPC_MPJ(fitness_arr,no_of_processes);
      fitness_arr = mpjobj.getList();
      for (indx = 0; indx < fitness_arr.length; indx++) 
      {
        if(fitness_arr[indx] != 0.0f)
        {
          result.addAll(fitness_applist.get(fitness_arr[indx]));  
        }    
      }
        return result;
    }
//---------------------------------------- calculate fitness for each application-----------------------
  ...
  private void assignContainers(SchedulerNode node) {
    ...
    // obtain the SchedulerApp list according to fitness
     List<SchedulerApp> fitness_sorted_list =  get_fitness_sorted_Applications(node);

    for (SchedulerApp application : fitness_sorted_list)
     {
      ...
      synchronized (application) {
        for (Priority priority : application.getPriorities()) {
          int maxContainers = 
            getMaxAllocatableContainers(application, priority, node,NodeType.OFF_SWITCH); 
